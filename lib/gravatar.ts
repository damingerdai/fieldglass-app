 ...
 */
const gravatar = (email: string, size: number = 100, defaultImage: string = 'identicon'): string => {
    ...
    const hash: string = crypto.createHash('md5').update(normalizedEmail).digest('hex');
 ...
 */
const advancedGravatar = (email: string, options: GravatarOptions = {}): string => {
    const {
        size = 100,
