...
import {
  DeleteLeaveEntitlementsButton,
  EditAmountSheet
} from '@/components/leave-entitlements';
...
export default async function Page() {
  ...
  const totalDays = entitlements.reduce(
    (acc, curr) => acc + curr.amount_days,
    0
  );
  const recentActivity =
    entitlements.length > 0 ? entitlements[0].updated_at : null;
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {recentActivity
                ? format(new Date(recentActivity), 'MMM d')
                : 'N/A'}
            </div>
            <p className="text-xs text-muted-foreground mt-1">
              Recent record modification
            </p>
          </CardContent>
        </Card>
      </div>
