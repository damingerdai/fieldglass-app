...
export const createLeaveEntitlementsSchema = z
  .object({
    leave_type: z.string().min(1, 'Please select a leave type'),
    amount_days: z.coerce
      .number<number>()
      .min(0.5, 'Amount must be at least 0.5 days'),
...
export type CreateLeaveEntitlementsSchema = z.infer<
  typeof createLeaveEntitlementsSchema
>;
